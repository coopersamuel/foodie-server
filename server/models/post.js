...
PostSchema.post('save', async function(doc, next) {
    ...
    await Restaurant.findById({ "_id": doc.restaurant }, async function(err, obj) { 
         await Restaurant.findByIdAndUpdate(doc.restaurant, { $set: { avgRating: ((obj.toObject().ratingCount * obj.toObject().avgRating) + doc.review)/(obj.toObject().ratingCount + 1) } }
    )});
    
    // ratingCount
    await Restaurant.findByIdAndUpdate(doc.restaurant, { $inc: { ratingCount: 1 } });
    next();
});
